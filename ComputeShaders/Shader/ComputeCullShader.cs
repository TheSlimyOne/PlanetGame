...
{   
    ...
    public ComputeCopyShader ComputeCopyShader { get; set; }
    ...
    public ComputeCullShader(string shaderFilePath, ref RenderingDevice rd) : base(shaderFilePath, ref rd)
    {
        SetupComputeShader();
    }
       ...
       _computeShaderUniforms = new System.Collections.Generic.Dictionary<BufferNames, ComputeShaderUniform>()
		{
			...
			[BufferNames.INDICES] = new StorageBufferUniform(_rd, (int)BufferNames.INDICES,
				Utilities.ToBytes<uint>(new uint[] { 0, 1, 8, (uint)PlanetController.PlanetData.MaximumNodes }).ToArray()
			),
			...
			[BufferNames.HEIGHT_MAP] = new TextureUniform(_rd, (int)BufferNames.HEIGHT_MAP,
				PlanetController.PlanetData.HeightMap),
    ...
    public override void Ready()
    {
		...
		_rd.ComputeListDispatchIndirect(computeList, ComputeCopyShader.GetUniformRid(ComputeCopyShader.BufferNames.DISPATCH_BUFFER), 0);
		_rd.ComputeListEnd();
    }
