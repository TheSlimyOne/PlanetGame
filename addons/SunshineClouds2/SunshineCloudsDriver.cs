...
public partial class SunshineCloudsDriver : Node
{
    ...
    private void RetrieveTextureData()
	{
		if (_cloudsResource != null)
		{
            ...
            {
                for (int i = 0; i < _trackedDirectionalLights.Count - _trackedDirectionalLightShadowSteps.Count; i++)
                {
                    _trackedDirectionalLightShadowSteps.Add(12);
                }
            }
			else if (_trackedDirectionalLightShadowSteps.Count > _trackedDirectionalLights.Count)
			{
				_trackedDirectionalLightShadowSteps = [.. _trackedDirectionalLightShadowSteps.ToList().GetRange(0, _trackedDirectionalLights.Count)];
            }
