...
vec4 radialBlurColor(vec4 startColor, sampler2D image, vec2 uv, float Directions, float blurVertical, float blurHorizontal, float Quality){
    float Pi = 6.28318530718;
    float count = 1.0;
	float theoreticalMaxCount =  Directions * Quality;
	float stepLerp = 1.0 / theoreticalMaxCount;
    ...
    for( float d=0.0; d<Pi; d+=Pi/Directions)
    {
		for(float i=1.0/Quality; i<=1.0; i+=1.0/Quality)
        {
			Color += texture(image, uv + vec2(cos(d) * blurHorizontal * i, sin(d) * blurVertical * i));
			count += 1.0;
        }
    }
    Color /= count;
...
void main() {
	...
	vec4 currentAccumilation = texture(input_color_image, accumUV);
	vec4 currentColorData = texture(input_data_image, accumUV);
	...
	if (blurPower > 0.0){
		...
		currentAccumilation = radialBlurColor(currentAccumilation, input_color_image, accumUV, blurQuality * 4.0, blurVertical, blurHorizontal, blurQuality);
	...
	if (directionalLightCount > 0.0){
		for (float i = 0.0; i < directionalLightCount; i++){
			...
			if (lightPower > 0.0){
				vec4 atmosphericData = sampleAllAtmospherics(rayOrigin, raydirection, linear_depth, traveledDistance, density, linear_depth / 10.0, 10.0, atmosphericDensity, sundir, light.color.rgb * lightPower, ambientfogdistancecolor);
				color.rgb = mix(color.rgb, atmosphericData.rgb, (atmosphericData.a * lightPower) / directionalLightCount); //causes jitter in the sky
			}
		}
	}
