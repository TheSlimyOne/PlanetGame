...
public partial class CameraController : Node3D
{
	...
	public string CurrentCameraName { get; private set; } = "None";
	...
	public override void _Input(InputEvent @event)
	{
		if (Debug && Input.IsActionJustPressed("switch_cam"))
		{
			...
			if (CurrentCameraName == "None")
				return;

			var cameraNames = new System.Collections.Generic.List<string>(Cameras.Keys);
			int currentIndex = cameraNames.IndexOf(CurrentCameraName);
	...
	public void SetCurrent(string cameraName)
	{
		...
		Cameras[CurrentCameraName].MakeCurrent();
		GD.Print($"Switched to camera: {CurrentCameraName} at {Cameras[CurrentCameraName].GlobalPosition}");
	}

	public CustomCamera GetCurrent() => Cameras[CurrentCameraName];
