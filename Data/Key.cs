...
namespace PlanetGame.Rendering.Surface
{
    ...
    public readonly struct TriangleData(Vector3 origin, Vector3 xNeighbor, Vector3 yNeighbor, Vector3 v0, Vector3 v1, Vector3 v2)
    {
        ...
        public readonly Vector3 XNeighbor = xNeighbor;
        public readonly Vector3 YNeighbor = yNeighbor;
    ...
    public readonly struct Key(uint msb, uint lsb, uint meshPolygonID, uint meshData)
    {
        ...
        public readonly Vector3 GetPolygonSpacePoint(Vector2 point) => QuadtreeSpaceToPolygonSpace() * VectorUtils.ToVector3(point, 1);
        ...
        public readonly Basis QuadtreeSpaceToPolygonSpace()
        {
            Vector3[] basePrimitive = GetBasePrimitive();

            return new Basis(
                basePrimitive[0] - basePrimitive[2],
                basePrimitive[1] - basePrimitive[2],
                basePrimitive[2]
            );
        }

        public readonly Vector3[] GetBasePrimitive()
        {
            ...
            return [
                (l0 * axisA) + (l1 * axisB) + normal,
            (r0 * axisA) + (r1 * axisB) + normal,
            normal
            ];
        }

        public readonly Key GetParentKey()
        {
            uint[] parent = RightShift64(GetNodeID(), 2);
            return new Key(parent[0], parent[1], MeshPolygonID, MeshData);
        }

        public readonly Key[] GetChildKeys()
        {
            uint[] baseKey = LeftShift64(GetNodeID(), 2);

            return [
                new Key(baseKey[0], baseKey[1] | 0u, MeshPolygonID, MeshData),
            new Key(baseKey[0], baseKey[1] | 1u, MeshPolygonID, MeshData),
            new Key(baseKey[0], baseKey[1] | 2u, MeshPolygonID, MeshData),
            new Key(baseKey[0], baseKey[1] | 3u, MeshPolygonID, MeshData)
            ];
        }

        public readonly bool IsUpperLeftChild() => (LSB & 3u) == 0u;

        public readonly TriangleData GetTriangle()
        {
            Basis quadtreeSpace = LeafSpaceToQuadtreeSpace();

            Vector2 pointA = VectorUtils.toVector2(VectorUtils.ToVector3(new Vector2(0.5f, 0.5f), 1) * quadtreeSpace);
            Vector2 pointB = VectorUtils.toVector2(VectorUtils.ToVector3(new Vector2(0.5f, -0.5f), 1) * quadtreeSpace);
            Vector2 pointC = VectorUtils.toVector2(VectorUtils.ToVector3(new Vector2(-0.5f, 0.5f), 1) * quadtreeSpace);

            Vector2 pointD = VectorUtils.toVector2(VectorUtils.ToVector3(Vector2.Zero, 1) * quadtreeSpace);
            Vector2 pointE = VectorUtils.toVector2(VectorUtils.ToVector3(Vector2.Right, 1) * quadtreeSpace);
            Vector2 pointF = VectorUtils.toVector2(VectorUtils.ToVector3(Vector2.Down, 1) * quadtreeSpace);

            Basis polygonSpace = QuadtreeSpaceToPolygonSpace();

            return new TriangleData(
                polygonSpace * VectorUtils.ToVector3(pointA, 1),
                polygonSpace * VectorUtils.ToVector3(pointB, 1),
                polygonSpace * VectorUtils.ToVector3(pointC, 1),
                polygonSpace * VectorUtils.ToVector3(pointD, 1),
                polygonSpace * VectorUtils.ToVector3(pointE, 1),
                polygonSpace * VectorUtils.ToVector3(pointF, 1)
            );
        }
        ...
        public readonly Mesh CreateTriangle()
        {
            TriangleData triangleData = GetTriangle();

            Vector3 vertexA = triangleData.V0;
            Vector3 vertexB = triangleData.V1;
            Vector3 vertexC = triangleData.V2;

            Vector3 normal = VectorUtils.GetTriangularNormal([vertexA, vertexB, vertexC]);

            Godot.Collections.Array arrays = [];
            arrays.Resize((int)Mesh.ArrayType.Max);
            arrays[(int)Mesh.ArrayType.Vertex] = new Vector3[] { vertexA, vertexB, vertexC };
